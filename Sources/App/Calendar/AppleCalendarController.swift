...
// `AppleSyncResponse` is the shared wire result for every selective-sync
// ingest endpoint (LuminaVaultShared v0.69.0); the server-side
// `ResponseEncodable` conformance lives here so the handler can return it
// directly, mirroring the Health ingest controller.
        ...
        for input in body.events {
            ...
            VALUES
                (\(bind: tenantID), \(bind: Self.source), \(bind: externalID), \(bind: input.calendarID),
                 ...
                 \(bind: status), \(bind: input.organizer), \(bind: input.remoteUpdatedAt),
                 NOW(), NOW())
