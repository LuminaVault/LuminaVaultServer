...
struct M89_CreateParallelExecutions: AsyncMigration {
    func prepare(on database: any Database) async throws {
        ...
        """#).run()
        try await sql.raw("CREATE INDEX IF NOT EXISTS conversation_messages_parallel_execution_idx ON conversation_messages(parallel_execution_id)").run()
        try await sql.raw(#"""
        ...
        """#).run()
        try await sql.raw("CREATE INDEX IF NOT EXISTS router_outputs_execution_idx ON router_outputs(execution_id, round, occurred_at)").run()
        try await sql.raw(#"""
        ...
        """#).run()
        try await sql.raw("CREATE INDEX IF NOT EXISTS router_synthesis_presets_tenant_idx ON router_synthesis_presets(tenant_id, updated_at DESC)").run()
    }

    func revert(on database: any Database) async throws {
        guard let sql = database as? any SQLDatabase else { return }
        try await sql.raw("DROP TABLE IF EXISTS router_synthesis_presets").run()
        try await sql.raw("DROP TABLE IF EXISTS router_outputs").run()
        try await sql.raw("DROP INDEX IF EXISTS conversation_messages_parallel_execution_idx").run()
        try await sql.raw("ALTER TABLE conversation_messages DROP COLUMN IF EXISTS parallel_execution_id").run()
        try await sql.raw(#"""
        ALTER TABLE router_executions
