...
## Fire-and-forget in route handlers: `Task` vs `Task.detached`

In server route handlers (Hummingbird, Vapor), fire-and-forget work uses the wrong task type 80% of the time.

**The rule:** if the work is *best-effort but should be cancelled if the client disconnects*, use `Task {}` — not `Task.detached`. When the HTTP response completes or the connection drops, the handler's task is cancelled; any child `Task {}` inherits that cancellation automatically.

```swift
// BAD: detached keeps running even if the client disconnects
Task.detached {
    await apns.notify(...)
}

// GOOD: inherits request task context, cancelled on disconnect
Task {
...
- The work MUST outlive the request (e.g., background job that the client triggers and doesn't wait for)
- It's genuinely CPU-bound and would starve the cooperative pool
- You explicitly need to break actor isolation AND you handle cancellation manually

**Common best-effort side effects that should use `Task {}`:** APNS push, achievement tracking, analytics, event bus publish, audit logging. All of these are "nice to complete but meaningless if the user already left."
