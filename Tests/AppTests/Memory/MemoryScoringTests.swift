...
/// No DB / Hermes needed.
@Suite struct MemoryScoringTests {

    @Test
    func freshUntouchedMemoryScoresOne() {
    ...
    @Test
    func recencyDecaysAtHalflife() {
        let now = Date()
        let halflifeAgo = now.addingTimeInterval(-30 * 86_400)
        let score = MemoryScoring.compute(
            accessCount: 0,
            queryHitCount: 0,
    ...
    @Test
    func accessAndQueryHitsDominateOldRow() {
        let now = Date()
        let veryOld = now.addingTimeInterval(-365 * 86_400)
    ...
    @Test
    func zeroAgeNeverNegative() {
        // Future `createdAt` (clock skew) must not produce age<0 → exp(>0) blowup.
        let now = Date()
        let future = now.addingTimeInterval(86_400)   // 1 day in the future
        let score = MemoryScoring.compute(
            accessCount: 0,
            queryHitCount: 0,
            createdAt: future,
