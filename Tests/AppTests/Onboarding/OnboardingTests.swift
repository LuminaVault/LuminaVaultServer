...
struct OnboardingTests {
    ...
    @Test
    func getReturnsFreshStateForNewUser() async throws {
    ...
    @Test
    func patchSetsFlagAndTimestamp() async throws {
        let app = try await buildApplication(reader: dbTestReader)
        try await app.test(.router) { client in
            let (token, _, _) = try await Self.register(client: client)
    ...
    @Test
    func patchIsIdempotentDoesNotOverwriteTimestamp() async throws {
        let app = try await buildApplication(reader: dbTestReader)
        try await app.test(.router) { client in
            let (token, _, _) = try await Self.register(client: client)
    ...
    @Test
    func patchRejectsFalseValues() async throws {
        let app = try await buildApplication(reader: dbTestReader)
        try await app.test(.router) { client in
            let (token, _, _) = try await Self.register(client: client)
    ...
    @Test
    func getIsStableAcrossCalls() async throws {
        let app = try await buildApplication(reader: dbTestReader)
        try await app.test(.router) { client in
            let (token, _, _) = try await Self.register(client: client)
