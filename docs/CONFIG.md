...
| `FREELANE_OPEN_ROUTER_MODEL` | `nvidia/nemotron-3-ultra-550b-a55b:free` | Leg 1 primary, on the platform OpenRouter key. 1M context, $0, tool-calling. |
| `FREELANE_OPEN_ROUTER_SECONDARY_MODEL` | `nvidia/nemotron-3-super-120b-a12b:free` | Leg 1 second hop — same key, **same daily counter**. 262K context. Resilience against one model being down or retired (a 404 on the lane advances to the next rung), not extra capacity. |
